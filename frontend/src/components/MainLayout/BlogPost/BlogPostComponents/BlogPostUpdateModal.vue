    ...
    <div class="tinymce-wrapper">
        <div class="tinymce-header">
            <div class="row position-relative">
                ...
                <!-- Nút lưu bài viết -->
                <button @click="cancelPost" class="cancelPost position-absolute">Hủy</button>
                <button @click="savePost" class="savePost position-absolute">Cập nhật</button>
            </div>
    
            <div class="title-wrapper d-flex">
                <label for="" class="labelPost">Tiêu đề:</label>
            </div>
            <textarea spellcheck="false" @input="autoResize($event.target)" v-model="titleArticle" class="inputTitle" rows="1"
...
<script>
    import {searchOwner, uploadImagePostToCloudinary, articleUpdate} from '@/services/api'; // Import hàm gọi API từ api.js
    export default {
        name: 'BlogPostModal',
        ...
        data() {
            return {
                ...
                inputsearchHost: "",
                hostResults: [],
                localArticle: { ...this.selectedArticle },
            };
        },
        mounted() {
            if (!this.categoryId) {
        ...
        methods: {
            initializeTinyMCE() {
                window.tinymce.init({
                    ...
                    setup: (editor) => {
                            editor.on('init', () => {
                            if (this.localArticle) {
                                // Gán tiêu đề vào TinyMCE editor
                                editor.setContent(this.localArticle.content || '');
    ...
    if (owner) {
        this.ownerName = owner.fullName || '';  // Cập nhật nếu có giá trị
        this.ownerAvatar = owner.profilePicture || '';  // Cập nhật nếu có giá trị
        this.ownerId = owner._id || '';  // Cập nhật nếu có giá trị
        this.input_select = false;  // Cập nhật trạng thái
    }
}
                            }
                            });
            ...
            cancelPost() {
            // Phát ra sự kiện đóng modal cho component cha
            this.$emit('close-modal'); // Gửi sự kiện 'close-modal' lên component cha
            },
            onPostTypeChange() {
